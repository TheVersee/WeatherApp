...
class App extends Component {
  ...
  commentBox = () =>{
    return(
      ...
      value={this.state.comment} placeholder="Enter your comment here..." /><br />
        ...
        <div>
          <span className="spanCmnt">Comments({this.state.listOfComments.length})</span><br/>
          
          {
            this.state.listOfComments.map(eachElm =>(
              <div className="listCmnt"><img src="https://png.pngtree.com/png-vector/20190423/ourmid/pngtree-user-icon-vector-illustration-in-glyph-style-for-any-purpose-png-image_975597.jpg" /> <p><b>{eachElm}</b></p></div>
            ))
          }
  ...
  find = async() =>{
    const url=`https://api.openweathermap.org/data/2.5/weather?q=${this.state.city}&APPID=7aeda93b35e682da294149685d9fe4f0`;
    //console.log(url);
...
/*function Bar(){
  return (
    <div className="bar">
      ...
      <button onChange={()=>this.getInput(event)} value={this.state.name}>Search</button>
    </div>
    );
}*/
...
//<img src="https://png.pngtree.com/png-vector/20190423/ourmid/pngtree-user-icon-vector-illustration-in-glyph-style-for-any-purpose-png-image_975597.jpg" />
